...
are_mutually_exclusive <- function(phenotype.a, phenotype.b, studies, verbose=FALSE){
    ...
    mbm <- microbenchmark("charstates()" = {charstates(c(phenotype.a, phenotype.b))},
                          "filter-by-studies"={character_states[(character_states$phenotype.id == phenotype.a) & (character_states$study.id %in% studies), ]},
                          "get-states"={charstates.a$state.id},
                          "get-unique-chars"={unique(charstates.a$character.id)},
                          "char-intersection"={sum(chars.a %in% chars.b)},
                          "char-state intersection"={intersection_length(charstates.a, charstates.b)}
                          )
    
    list(me=mutually_exclusive, mbm=mbm)
}

result <- are_mutually_exclusive(phenotypes[2], phenotypes[3], studies=study)
autoplot(result$mbm)
