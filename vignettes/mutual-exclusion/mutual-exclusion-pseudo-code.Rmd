...
are_mutually_exclusive <- function(phenotype.a, phenotype.b, studies){
  ...
  states.a <- sapply(states.a, filter_states(studies))
  states.b <- sapply(states.b, filter_states(studies))
  ...
  if((length(studies.a) > 0) & (length(studies.b) > 0)){ # determine mutual exclusivity only if both studies are non-empty
    ...
    char_states.a <- vector(states.a, get_char_states)
    char_states.b <- vector(states.b, get_char_states)
    ...
    error("At least one of the phenotypes is not part of any studies.")
...
get_char_state_match_count <- function(char_states.a, char_states.b, state_count=FALSE){
    ...
    if(state_count){return(state_different_count)} else {return(char_match_count)}
}
